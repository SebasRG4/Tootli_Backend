...
use App\Models\StoreGalleryMedia;
use App\Models\StoreMenuMedia;
...
{
    // Validación de los campos del formulario de admin
    $validator = Validator::make($request->all(), [
        ...
        'gallery.*' => 'image|mimes:webp,jpg,png,jpeg,gif,bmp,tif,tiff|max:2048',
        'menu_photo.*' => 'nullable|image|mimes:webp,jpg,png,jpeg,gif,bmp,tif,tiff|max:2048',
        'serves_alcohol' => 'required|in:0,1',
        'average_cost_per_person' => 'nullable|numeric|min:0',
        'description' => 'required|string|max:1000',
        'has_guarantee_seal' => 'required|in:0,1',
    ], [
        'f_name.required' => translate('messages.first_name_is_required'),
        'name.0.required'=>translate('default_name_is_required'),
        'category_ids.required' => translate('Porfavor selecciona una categoria'),
        'serves_alcohol.required' => 'Indica si sirve alcohol.',
    'description.required' => 'La descripción es obligatoria.',
    ]);

    // ... (El resto de tus validaciones de zona, etc.) ...
    if ($validator->fails()) {
    ...
    $store->serves_alcohol = $request->serves_alcohol;
    $store->average_cost_per_person = $request->average_cost_per_person;
    $store->description = $request->description;
    $store->has_guarantee_seal = $request->has_guarantee_seal;
    $store->save();

    // Lógica para guardar la galería y el menú
    try {
        $store->categories()->attach($request->category_ids);

        if ($request->hasFile('gallery')) {
            foreach ($request->file('gallery') as $img) {
                $path = $img->store('public/store/gallery');
                $store->galleryMedia()->create([
                    'file_path' => $path,
                    'type' => 'image',
                    'display_order' => $store->galleryMedia()->count(),
                ]);
            }
        }

        if ($request->hasFile('menu_photo')) {
            $path = $request->file('menu_photo')->store('public/store/menu');
            $store->menuMedia()->create([
                'file_path' => $path,
                'type' => 'image',
            ]);
        }
        
        // ... (resto de tu código de traducciones, etc.) ...
    ...
    // Mostrar galería y menú del store
public function showGallery($vendorId)
{
    $vendor = Vendor::findOrFail($vendorId);
    $store = $vendor->stores()->first();
    $gallery = $store->galleryMedia; // relación definida en el modelo Store
    $menu = $store->menuMedia; // relación definida en el modelo Store
    return view('admin-views.vendor.gallery', compact('vendor', 'store', 'gallery', 'menu'));
}

// Añadir imagen a galería
public function addGalleryImage(Request $request, $vendorId)
{
    $request->validate([
    ...
    $vendor = Vendor::findOrFail($vendorId);
    $store = $vendor->stores()->first();
    $path = $request->file('image')->store('public/store/gallery');

    $store->galleryMedia()->create([
        'file_path' => $path, // Usamos file_path
        'caption' => $request->caption,
        'type' => 'image',
        'display_order' => $store->galleryMedia()->count(),
    ]);

    return redirect()->route('admin.vendor.gallery', $vendorId)->with('success', 'Imagen añadida');
}

// Eliminar imagen de galería  
public function deleteGalleryImage($vendor_id, $image_id)
{
    // Usamos el modelo correcto: StoreGalleryMedia
    $imageToDelete = \App\Models\StoreGalleryMedia::find($image_id);

    if (!$imageToDelete) {
        return response()->json(['success' => false, 'message' => 'Imagen no encontrada.'], 404);
    }

    // Obtenemos el ID de la tienda antes de borrar
    $store_id = $imageToDelete->store_id;

    // Borramos el archivo físico del almacenamiento
    if ($imageToDelete->file_path) {
        \Illuminate\Support\Facades\Storage::delete($imageToDelete->file_path);
    }

    // Borramos el registro de la base de datos
    $imageToDelete->delete();

    // --- ¡LA PARTE MÁS IMPORTANTE EMPIEZA AQUÍ! ---
    // Después de borrar, re-indexamos el display_order de las imágenes restantes.
    DB::transaction(function () use ($store_id) {
        // 1. Obtenemos todas las imágenes que quedan para esa tienda.
        $remainingGalleryImages = \App\Models\StoreGalleryMedia::where('store_id', $store_id)
                                         ->orderBy('display_order', 'asc')
                                         ->get();

        // 2. Recorremos las imágenes restantes y les asignamos un nuevo índice secuencial.
        $i = 0;
    ...
    return response()->json(['success' => true, 'message' => 'Imagen eliminada y orden actualizado.']);
}

// Eliminamos imagen de menú
public function deleteMenuImage($vendorId, $imageId)
{
    $vendor = \App\Models\Vendor::findOrFail($vendorId);
    $store = $vendor->stores()->first();
    $image = $store->menuMedia()->findOrFail($imageId);
    \Illuminate\Support\Facades\Storage::delete(str_replace('/storage/', 'public/', $image->url));
    $image->delete();

    return response()->json(['success' => true]);
}

public function reorderGallery(\Illuminate\Http\Request $request, $vendorId)
{
    $vendor = \App\Models\Vendor::findOrFail($vendorId);
    $store = $vendor->stores()->first();

    // $request->order es un array con los IDs en el nuevo orden
    foreach ($request->order as $index => $id) {
        $media = $store->galleryMedia()->find($id);
        if ($media) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'latitude' => 'required',
            'longitude' => 'required',
            'gallery.*' => 'nullable|image|mimes:webp,jpg,png,jpeg,gif,bmp,tif,tiff|max:2048',
            'menu_photo.*' => 'nullable|image|mimes:webp,jpg,png,jpeg,gif,bmp,tif,tiff|max:2048',
            'description' => 'required|string|max:1000',
        ]);
        ...
        $store->address = $request->address[0];
        $store->tax = $request->tax;
        $store->latitude = $request->latitude;
        $store->longitude = $request->longitude;
        $store->average_cost_per_person = $request->average_cost_per_person ?? $store->average_cost_per_person;
        $store->serves_alcohol = $request->serves_alcohol ?? $store->serves_alcohol;
        $store->has_guarantee_seal = $request->has_guarantee_seal ?? $store->has_guarantee_seal;
        $store->description = $request->description;
    
        // Logo
        if ($request->hasFile('logo')) {
            $store->logo = \App\CentralLogics\Helpers::update('store/', $store->logo, 'png', $request->file('logo'));
        }
        // Cover
        if ($request->hasFile('cover_photo')) {
            $store->cover_photo = \App\CentralLogics\Helpers::update('store/cover/', $store->cover_photo, 'png', $request->file('cover_photo'));
        }
    
        // Galería (múltiples) - CORREGIDO
        if ($request->hasFile('gallery')) {
            foreach ($request->file('gallery') as $img) {
                $path = $img->store('public/store/gallery');
                $store->galleryMedia()->create([
        ...
        // Menú (multiples imagenes)
        if ($request->hasFile('menu_photo')) {
            foreach ($request->file('menu_photo') as $img) {
                $path = $img->store('public/store/menu');
                $store->menuMedia()->create([
                    'file_path' => $path,
                    'type' => 'image',
                    'display_order' => $store->menuMedia()->count(),
                ]);
            }
        }
        ...
        \Brian2694\Toastr\Facades\Toastr::success('Tienda actualizada correctamente');
