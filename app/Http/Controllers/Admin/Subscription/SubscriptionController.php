...
use App\Models\SubscriptionFeature; // <--- Importación ya presente, ¡perfecto!
...
class SubscriptionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $module= $request->module ?? 'all'; // <- MODIFICACION: Añadido para evitar error
        $features = SubscriptionFeature::all();
        // Se añade 'module' al compact, que faltaba en tu código original y podría dar error.
        return view('admin-views.subscription.package.create', compact('language','module', 'features'));
    }
    public function store(Request $request)
    {
        $request->validate([
            ...
            'feature_ids' => 'nullable|array' // <- Tu validación ya estaba, ¡genial!
        ], [
        ...
        // --- MODIFICACIÓN ---
        // Sincroniza las features seleccionadas en el formulario de creación.
    ...
    {
        $subscriptionackage->load('translations', 'features')->withoutGlobalScope('translate'); // Carga las features
        $language = getWebConfig('language');
        $defaultLang = str_replace('_', '-', app()->getLocale());
        // --- MODIFICACIÓN ---
        // Obtenemos todas las features para poder listarlas en el formulario
        $features = SubscriptionFeature::all();
        // --- FIN MODIFICACIÓN ---
        return view('admin-views.subscription.package.edit', compact('language','defaultLang','subscriptionackage', 'features'));
    }
    ...
    public function update(SubscriptionPackage $subscriptionackage, Request $request)
    {
        ...
        $request->validate([
            ...
            'review' => 'nullable|boolean',
            'text' => 'nullable|max:1000',
            'feature_ids' => 'nullable|array' // <-- MODIFICACIÓN: Añadida validación
        ], [
            'price.required' => translate('Must enter Price for the Package'),
            'package_validity.required' => translate('Must enter a validity period for the Package in days'),
            'package_validity.between' => translate('validity must be in 99 years'),
        ...
        // --- MODIFICACIÓN ---
        // Sincroniza las features. Si no se envía ninguna, se desasocian todas.
        $subscriptionackage->features()->sync($request->feature_ids ?? []);
        // --- FIN MODIFICACIÓN ---
    ...
    // ... (overView, transaction, settings, etc.)
