    ...
    public function offlinePayments(Request $request)
{
    ...
    $offlinePayments = \App\Models\OfflinePayments::where('delivery_man_id', $dm->id)
        ->orderBy('created_at', 'desc')
        ->get(['id', 'amount', 'payment_method_id', 'payment_info', 'customer_note', 'created_at']);

    return response()->json(['offline_payments' => $offlinePayments], 200);
}
    ...
    public function submit_offline_payment(Request $request)
{
    ...
    $deliveryman = DeliveryMan::where('auth_token', $request['token'])->firstOrFail();

    $evidencePath = null;
    if ($request->hasFile('evidence')) {
        $evidencePath = $request->file('evidence')->store('deliveryman_offline_payments');
    }

    $offlinePayment = DeliverymanOfflinePayment::create([
    ...
    return response()->json(['message' => 'Pago registrado, pendiente de aprobación.'], 200);
}
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        if (!$dm->can_accept_cash) {
            $orders = $orders->where('payment_method', '!=', 'cash_on_delivery');
        }
    ...
    public function accept_order(Request $request)
{
    ...
    // SOLO EL REPARTIDOR RESERVADO PUEDE ACEPTAR
    if ($order->reserved_delivery_man_id !== $dm->id) {
        return response()->json([
            'errors' => [
                ['code' => 'not_reserved', 'message' => 'Este pedido no está reservado para ti.']
            ]
        ], 403);
    }

    // Ya fue aceptado por otro repartidor
    if ($order->delivery_man_id !== null) {
        return response()->json([
            'errors' => [
                ['code' => 'already_assigned', 'message' => 'El pedido ya fue asignado a otro repartidor.']
            ]
        ], 409);
    }
    ...
    // Validación de balance efectivo
    ...
    $order->accepted = now();
    $order->reserved_delivery_man_id = null; // Limpiar la reserva
    $order->save();
