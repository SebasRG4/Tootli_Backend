    ...
    if ($user && $user->wallet_balance < 0) {
        return response()->json([
            'errors' => [
                [
                    'code' => 'negative_funds',
                    'message' => 'Debes tener saldo en 0 o positivo para hacer un nuevo pedido. Por favor paga tu saldo pendiente por el método offline disponible.'
                ]
            ]
        ], 403);
    }
        ...
        try {
            ...
            try {
                // Llama al servicio que realiza la asignación automática de repartidor
                app(OrderAssignmentService::class)->assignOrderToBestCandidate($order);
            } catch (\Throwable $ex) {
                \Log::error('Error en asignación automática de pedido: ' . $ex->getMessage());
            }
