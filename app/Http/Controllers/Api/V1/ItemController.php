...
class ItemController extends Controller
{
    ...
    public function get_seasonal_products(Request $request)
{
    if (!$request->hasHeader('zoneId')) {
        $errors = [];
        array_push($errors, ['code' => 'zoneId', 'message' => translate('messages.zone_id_required')]);
    ...
    $productLogic = new \App\CentralLogics\ProductLogic();
    
    $items = $productLogic->get_seasonal_products($request);
...
/**
 * Obtener combos y kits
 */
public function get_combo_kit_products(Request $request)
{
    if (!$request->hasHeader('zoneId')) {
        $errors = [];
        array_push($errors, ['code' => 'zoneId', 'message' => translate('messages.zone_id_required')]);
    ...
    $items = $productLogic->get_combo_kit_products($request);
...
/**
 * Obtener productos especiales (temporada + combos/kits en una sola llamada)
 */
public function get_special_products(Request $request)
{
    if (!$request->hasHeader('zoneId')) {
        $errors = [];
        array_push($errors, ['code' => 'zoneId', 'message' => translate('messages.zone_id_required')]);
    ...
    $seasonal_items = ProductLogic::get_seasonal_products($zone_id, $limit, $offset);
    $seasonal_products = Helpers::product_data_formatting($seasonal_items['products'], true, false, app()->getLocale());
    
    // Obtener combos/kits
    $combo_items = ProductLogic::get_combo_kit_products($zone_id, $limit, $offset);
    $combo_products = Helpers::product_data_formatting($combo_items['products'], true, false, app()->getLocale());
    
    return response()->json([
        'seasonal_products' => [
            'total_size' => $seasonal_items['total_size'],
            'products' => $seasonal_products
        ],
        'combo_kit_products' => [
            'total_size' => $combo_items['total_size'],
