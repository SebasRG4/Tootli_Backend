...
use Illuminate\Support\Facades\Storage as FileStorage; // Facade de archivos
use App\Models\Storage; // Modelo
   ...
   public function getLogoFullUrlAttribute() {
    ...
    if (!$value) return null;
        ...
        if ($this->storage && $this->storage->count() > 0) {
            $logoStorage = $this->storage->firstWhere('key', 'logo');
            if ($logoStorage) {
                $disk = $logoStorage['value'];
            }
    }

    // Retornar la URL corregida para tu hosting
    return url('storage/app/public/store/' . $value);
    }
    ...
    //Relaciones para tabals de imagenes/videos 
    // Relación para la galería del local
        public function galleryMedia()
        {
            return $this->hasMany(\App\Models\StoreGalleryMedia::class, 'store_id')->orderBy('display_order', 'asc');
        }
        
        // Relación para la multimedia del menú
        public function menuMedia()
        {
           return $this->hasMany(\App\Models\StoreMenuMedia::class, 'store_id');
        }
        
        // Relación para las categorías de la tienda
    ...
    {
        return $this->belongsToMany(SubscriptionFeature::class, 'store_subscription_features', 'store_id', 'subscription_feature_id')
     ...
     * @return HasOne
     */
    public function dineout()
    {
        return $this->hasOne(\App\Models\DineOut::class, 'store_id');
    }
