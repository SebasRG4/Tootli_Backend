...
class ProductLogic
{
       ...
       public function get_seasonal_products(Request $request)
    {
        try {
            ...
            // Debug para verificar el formato del zone_id
            \Log::info('Zone ID recibido:', ['zone_id' => $zone_id, 'type' => gettype($zone_id)]);
            ...
            $products = $query->skip(($offset - 1) * $limit)
                             ->take($limit)
                             ->get();
            ...
            \Log::error('get_seasonal_products error: ' . $e->getMessage());
            return [
   ...
   public function get_combo_kit_products(Request $request)
{
    try {
        ...
        \Log::error('get_combo_kit_products error: ' . $e->getMessage());
        return [
            'error' => 'Internal server error',
            'message' => $e->getMessage(),
            'total_size' => 0,
