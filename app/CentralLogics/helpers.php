    ...
    public static function product_data_formatting($data, $multi_data = false, $trans = false, $local = 'en' , $temp_product=false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['is_seasonal'] = (boolean)$item['is_seasonal'];
                $item['is_combo_kit'] = (boolean)$item['is_combo_kit'];
    ...
    public static function store_data_formatting($data, $multi_data = false)
{
    ...
    if ($multi_data == true) {
        foreach ($data as $item) {
            // 1. Cargar solo las relaciones que SÍ existen
            $item->load('storeConfig', 'dineout', 'categories', 'module');

            // 2. Usar el atributo 'rating' como lo hacía el código original (es un atributo, no una relación)
            $ratings = StoreLogic::calculate_store_rating($item['rating']);
            
            // 3. Convertir el objeto a un array para trabajar de forma segura
            $formatted_item = $item->toArray();

            // 4. Limpiar los datos que no queremos o que vamos a reemplazar
            unset($formatted_item['rating']); // Quitamos la colección de ratings crudos
            unset($formatted_item['storeConfig']); // La información ya se usó o se va a formatear
            unset($formatted_item['pivot']); // Dato inútil para el frontend

            // 5. Añadir los campos calculados y formateados al array
            $formatted_item['avg_rating'] = $ratings['rating'];
            $formatted_item['rating_count'] = $ratings['total'];
            $formatted_item['positive_rating'] = $ratings['positive_rating'];
            
            $formatted_item['is_recommended'] = ($item->storeConfig && $item->storeConfig->is_recommended_deleted == 0) ? $item->storeConfig->is_recommended : false;
            $formatted_item['halal_tag_status'] = (bool) $item->storeConfig?->halal_tag_status;
            
            $formatted_item['current_opening_time'] = self::getNextOpeningTime($item->schedules) ?? 'closed';

            // --- NUEVOS CAMPOS AÑADIDOS CORRECTAMENTE ---
            $formatted_item['has_guarantee_seal'] = (bool) $item->has_guarantee_seal;
            $formatted_item['serves_alcohol'] = (bool) $item->serves_alcohol;
            $formatted_item['average_cost_per_person'] = $item->average_cost_per_person;

            // Las relaciones ya vienen en el toArray, solo nos aseguramos de que tengan el formato deseado
            $formatted_item['categories'] = $item->categories->map(fn($cat) => ['id' => $cat->id, 'name' => $cat->name, 'slug' => $cat->slug])->toArray();
            $formatted_item['module'] = $item->module ? ['id' => $item->module->id, 'name' => $item->module->module_name] : null;
            
            // --- INFO DINEOUT ---
            $formatted_item['is_dineout'] = !is_null($item->dineout);
            $formatted_item['dineout_info'] = $item->dineout ? [
                'menu_descripcion' => $item->dineout->menu_descripcion,
                'latitude' => $item->dineout->latitude,
                'longitude' => $item->dineout->longitude,
                'has_sound' => (bool) $item->dineout->has_sound,
                'sound_url' => $item->dineout->sound_url,
                'has_guarantee_seal' => (bool) $item->dineout->has_guarantee_seal,
                'serves_alcohol' => (bool) $item->dineout->serves_alcohol,
            ] : null;
            
            array_push($storage, $formatted_item);
        }
        return $storage;
    } 
    // La parte 'else' para un solo item necesita una lógica similar. 
    // Si la necesitas, avísame y la adapto también.
    // Por ahora, la mayoría de tus endpoints usan el caso de $multi_data == true.
    else if ($data) {
        $data->load('storeConfig', 'dineout', 'categories', 'module');
        $ratings = StoreLogic::calculate_store_rating($data['rating']);
        $formatted_item = $data->toArray();
        unset($formatted_item['rating'], $formatted_item['storeConfig'], $formatted_item['pivot']);

        $formatted_item['avg_rating'] = $ratings['rating'];
        $formatted_item['rating_count'] = $ratings['total'];
        // ... (añadir el resto de campos como en el bloque if) ...
